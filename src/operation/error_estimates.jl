...
function (op::Operator{:Locking_ratio_mix})(apu::T,app::S) where {T<:AbstractElement,S<:AbstractElement}
    ΔW²= 0
    W̄² = 0
    Δu²= 0
    ū² = 0
    ...
    𝑤 = ξu.𝑤
        ...
        s₁₂ = 2G*ε̄₁₂
        s₃₃ = 2G*(-1/3*ε₁₁-1/3*ε₂₂)
        ΔW² += (3*(p-p̄)^2/2/K+(s₁₁-s̄₁₁)^2/4/G+(s₂₂-s̄₂₂)^2/4/G+(s₃₃-s̄₃₃)^2/4/G+(s₁₂-s̄₁₂)^2/4/G)^0.5*𝑤
    end
    return ΔW²
end 
function (op::Operator{:Locking_ratio_mix})(apsu::Vector{T},apsp::Vector{S}) where {T<:AbstractElement,S<:AbstractElement}
    HₑNorm_ΔW²= 0.0
...
function (op::Operator{:Hₑ_up_mix})(apu::T,app::S) where {T<:AbstractElement,S<:AbstractElement}
    ...
    𝑤 = ξu.𝑤
        ...
        # ΔW² += (3*(p-p̄)*(ε₁₁-ε̄₁₁+ε₂₂-ε̄₂₂)/2+(2/3*(ε₁₁-ε̄₁₁)-1/3*(ε₂₂-ε̄₂₂))*(s₁₁-s̄₁₁)/2+(-1/3*(ε₁₁-ε̄₁₁)+2/3*(ε₂₂-ε̄₂₂))*(s₂₂-s̄₂₂)/2+(-1/3*(ε₁₁-ε̄₁₁)-1/3*(ε₂₂-ε̄₂₂))*(s₃₃-s̄₃₃)/2+( ε₁₂- ε̄₁₂)*(s₃₃-s̄₃₃)/2)*𝑤
        # W̄² += (3*p*(ε₁₁+ε₂₂)/2+(2/3*ε₁₁-1/3*ε₂₂)*s₁₁/2+(-1/3*ε₁₁+2/3*ε₂₂)*s₂₂/2+(-1/3*ε₁₁-1/3*ε₂₂)*s₃₃/2+ ε₁₂*s₃₃/2)*𝑤
        Δu² += ((u₁ - ū₁)^2 + (u₂ - ū₂)^2)*𝑤
        ū² += (ū₁^2 + ū₂^2)*𝑤
        # p̄²  +=p̄^2*𝑤
        # p²  +=p^2*𝑤
