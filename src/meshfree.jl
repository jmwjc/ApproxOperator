...
@inline get𝒑₃(ap::ReproducingKernel{:Quartic1D},ξ::SNode) = get𝒑₃(ap,ξ.ξ)
@inline get𝒑₃(::ReproducingKernel{:Quartic1D},ξ::Float64) = (1.0,0.5*(1.0-ξ))
@inline get∂𝒑₃∂ξ(::ReproducingKernel{:Quartic1D},ξ::Any) = (0.0,1.0)
...
function cal∇𝗚₂!(ap::ReproducingKernel{:Cubic1D,𝑠,𝜙,:Seg2}) where {𝑠,𝜙}
    𝗚⁻¹ = ap.𝗠[:∇̃²]
    𝗚⁻¹∂ξ = ap.𝗠[:∂∇̃²∂ξ]
    fill!(𝗚⁻¹,0.0)
    fill!(𝗚⁻¹∂ξ,0.0)
...
function set∇³₁𝝭!(ap::ReproducingKernel,𝒙::SNode)
    ...
    ∂²𝝭∂x² = 𝒙[:∂²𝝭∂x²]
    ∂³𝝭∂x³ = 𝒙[:∂³𝝭∂x³]
    𝒑₀ᵀ𝗠⁻¹, 𝒑₀ᵀ∂𝗠⁻¹∂x, 𝒑₀ᵀ∂²𝗠⁻¹∂x², 𝒑₀ᵀ∂³𝗠⁻¹∂x³ = cal∇³₁𝗠!(ap,𝒙)
    for (i,𝒙ᵢ) in enumerate(𝓒)
        Δ𝒙 = 𝒙 - 𝒙ᵢ
        𝒑, ∂𝒑∂x, ∂²𝒑∂x², ∂³𝒑∂x³ = get∇³₁𝒑(ap,Δ𝒙)
        𝜙, ∂𝜙∂x, ∂²𝜙∂x², ∂³𝜙∂x³ = get∇³₁𝜙(ap,𝒙ᵢ,Δ𝒙)
        𝒑₀ᵀ𝗠⁻¹𝒑 = 𝒑₀ᵀ𝗠⁻¹*𝒑
        𝒑₀ᵀ∂𝗠⁻¹∂x𝒑 = 𝒑₀ᵀ∂𝗠⁻¹∂x*𝒑
        𝒑₀ᵀ𝗠⁻¹∂𝒑∂x = 𝒑₀ᵀ𝗠⁻¹*∂𝒑∂x
        𝒑₀ᵀ∂²𝗠⁻¹∂x²𝒑 = 𝒑₀ᵀ∂²𝗠⁻¹∂x²*𝒑
...
function set∇∇̃²𝝭!(gp::ReproducingKernel{𝒑,𝑠,𝜙,:Seg2},ap::ReproducingKernel{𝒑,𝑠,𝜙,:Seg2}) where {𝒑,𝑠,𝜙}
    ...
    for ξ̂ in 𝓖
        ...
        for ξ in ap.𝓖
            ...
            𝒒, ∂𝒒∂ξ, ∂²𝒒∂ξ² = get∇²𝒑₂(gp,ξ)
            W₁ = (𝒒̂ᵀ𝗚⁻¹*∂𝒒∂ξ*nᵇ₁*wᵇ + 𝒒̂ᵀ𝗚⁻¹*∂²𝒒∂ξ²*w)/𝐿 
            ∂W₁∂x = -(∂𝒒̂ᵀ∂ξ𝗚⁻¹*∂𝒒∂ξ*nᵇ₁*wᵇ + ∂𝒒̂ᵀ∂ξ𝗚⁻¹*∂²𝒒∂ξ²*w)/𝐿^2 
            W₁₁ = 𝒒̂ᵀ𝗚⁻¹*𝒒*nᵇ₁*wᵇ
            ∂W₁₁∂x = -∂𝒒̂ᵀ∂ξ𝗚⁻¹*𝒒*nᵇ₁*wᵇ/𝐿
            for i in 1:length(𝓒)
                ∂²𝝭∂x²[i] += 𝝭[i]*W₁ + ∂𝝭∂x[i]*W₁₁
                ∂∂²𝝭∂x²∂x[i] += 𝝭[i]*∂W₁∂x + ∂𝝭∂x[i]*∂W₁₁∂x
            end
        end
    end
end
...
function set∇∇̄²𝝭!(ap::ReproducingKernel{𝒑,𝑠,𝜙,:Seg2}) where {𝒑,𝑠,𝜙}
    x₁ = ap.𝓒[1].x;y₁ = ap.𝓒[1].y
    x₂ = ap.𝓒[2].x;y₂ = ap.𝓒[2].y
    𝐿 = ((x₁-x₂)^2+(y₁-y₂)^2)^0.5
    n₁ =  1.0
    ...
    for ξ̂ in 𝓖
        ...
        for ξ in 𝓖
            ...
            𝝭 = ξ[:𝝭]
            ∂𝝭∂x = ξ[:∂𝝭∂x]
            𝒒, ∂𝒒∂ξ, ∂²𝒒∂ξ² = get∇²𝒑₂(ap,ξ)
            W₁ = 𝒒̂ᵀ𝗚⁻¹*∂𝒒∂ξ*nᵇ₁*w/𝐿 
            ∂W₁∂x = -∂𝒒̂ᵀ∂ξ𝗚⁻¹*∂𝒒∂ξ*nᵇ₁*w/𝐿^2 
            W₁₁ = 𝒒̂ᵀ𝗚⁻¹*𝒒*nᵇ₁*w
            ∂W₁₁∂x = -∂𝒒̂ᵀ∂ξ𝗚⁻¹*𝒒*nᵇ₁*w/𝐿
            for i in 1:length(𝓒)
                # ∂²𝝭∂x²[i] += 𝝭[i]*W₁ + ∂𝝭∂x[i]*W₁₁
...
for set𝝭 in (:set𝝭!,:set∇𝝭!,:set∇₁𝝭!,:set∇₂𝝭!,:set∇²𝝭!,:set∇²₁𝝭!,:set∇²₂𝝭!,:set∇³𝝭!,:set∇³₁𝝭!,:set∇̂³𝝭!)
    @eval begin
...
function set∇̄²𝝭!(aps::Vector{T};Γᵍ::Vector{T}=T[],Γᶿ::Vector{T}=T[],Γᴾ::Vector{T}=T[]) where T<:AbstractElement{:Tri3}
    for ap in aps
        set∇̄²𝝭!(ap,Γᵍ=Γᵍ,Γᶿ=Γᶿ,Γᴾ=Γᴾ)
    end
end

function set∇∇̄²𝝭!(aps::Vector{T}) where T<:AbstractElement{:Seg2}
    for ap in aps
        set∇∇̄²𝝭!(ap)
    end
end
